...
void Model::clearOperaContainer() {
  operaContainer.clear();
}
...
void Model::deleteOpera(Opera* oldOpera) {
  operaContainer.remove(oldOpera);
}

void Model::modifyOpera(Opera* oldOpera, Opera* newOpera) {
  deleteOpera(oldOpera);
  insertOpera(newOpera);
}
...
unsigned int Model::getCountSameAuthor(const QString &author) {
  unsigned int value = 0;
  for (int i = 0; i < operaContainer.getSize(); i++){
    if (operaContainer[i]->clone()->getAuthor() == author) value++;
  }
  return value;
}

DeepPtr<Opera> Model::searchOpera(int index) {
  return operaContainer[index];
}
